...
int main() {
  std::array<Student, 4> students = {Student{"Alice", 90}, Student{"Bob", 75},
                                     Student{"Charlie", 85},
                                     Student{"Dave", 70}};
  ...
  auto it = std::find_if(
      ...
      [&searchName](const Student& s) { return s.name == searchName; });

  if (it != students.end()) {
    std::cout << "Found student: " << it->name << " with grade " << it->grade
              << std::endl;
  } else {
  ...
  bool found = std::binary_search(students.begin(), students.end(), target,
                                  compareByGrade);
  std::cout << "Binary search for grade 85: " << (found ? "Found" : "Not Found")
            << std::endl;
